...
环境变量:
  ...
  REMOTE_DIR             可选，默认 /opt/website/opc-website/ai-nav
...
DEFAULT_REMOTE_DIR = "/opt/website/opc-website/ai-nav"
...
def step_sync(cfg: dict) -> None:
    ...
    rsync_cmd = [
        ...
        "--exclude=.DS_Store",
        "-e",
        f"ssh -i {cfg['ssh_key']} -p {cfg['port']} -o StrictHostKeyChecking=accept-new",
        f"{cfg['dist_dir']}/",
...
def main() -> int:
    print(f"{BOLD}========== ai-nav 一键部署 =========={RESET}")
    require_tools()
    cfg = load_config()
